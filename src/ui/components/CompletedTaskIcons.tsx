...
export function CompletedTaskIcons() {
  ...
  const completedEntries: { task: Task; key: string }[] = [];
    ...
    <div className={styles.container}>
      {completedEntries.map(({ task, key }) =>
        presentation === 'notes_rows' ? (
          <StickyNoteIndicator key={key} task={task} />
        ) : (
          <BlobIndicator key={key} task={task} />
        )
      )}
    </div>
  );
...
function BlobIndicator({ task }: { task: Task }) {
  ...
  return (
    <div className={styles.iconWrapper} data-tooltip={task.title}>
      <svg
        width={ICON_SIZE}
        height={ICON_SIZE}
